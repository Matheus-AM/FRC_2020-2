...
int main (int argc, char *argv[]) {
    ...
    */
    ...
    while(1){
        ...
        bzero ((char *)&newcli[connections], sizeof (newcli[connections]));
        newcli[connections].sin_family = AF_INET;
        newcli[connections].sin_port = htons(0);
        newcli[connections].sin_addr.s_addr = htonl(INADDR_ANY); 

        int cliLen = sizeof(newcli[connections]);
        int clientSock = accept(sock, (struct sockaddr *) &newcli[connections], &cliLen);
        if (clientSock == ERROR) {
            perror ("Accept");
            exit (0);
        }
